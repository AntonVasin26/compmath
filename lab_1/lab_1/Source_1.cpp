...
typedef std::vector <ftype> f_vector;
...
//	return answer;
//}
f_vector polynomial_remainder(const f_vector a, const f_vector b)
{
	auto m = std::size(b) - 1;
	auto n = std::size(a) - 1;
...
std::pair<ftype, ftype> localization(f_vector v)
{
	int n = v.size() - 1;
...
//typedef f_vector polinom;
//typedef std::pair<polinom, polinom> pair_f;

//f_vector minus(polinom p1, polinom p2)
//{
//	auto n1 = p1.size();
//	auto n2 = p2.size();
...
f_vector derivative_v(f_vector p0)
{
	f_vector der_p0;
	int m = p0.size();
	if (m == 0)
...
ftype count_sign_change(std::vector<f_vector> row, ftype value)
{
	int count = 0;
...
std::vector< std::pair<ftype, ftype>> sthurm_grid(
	std::vector<f_vector>row_sthurm, ftype first, ftype second, ftype eps)
{
...
ftype cal_D0(ftype Z)
{
	...
	ftype P_1 = powl(Z, n) * P_3;
	ftype y = P_1 / P_0;
	ftype q = ((gamma_0 + 1) + (gamma_0 - 1) * y) / ((gamma_0 - 1) + (gamma_0 + 1) * y);
	ftype d = P_0 / ro_0 * (y - 1) / (1 - q);
	ftype d0 = V_0 + pow(d, 0.5) * (y - 1) / std::abs(y - 1);
	return d0;
}
...
int main()
{
	...
	//ряд штурма
	std::vector<f_vector> row_sthurm;
	row_sthurm.push_back(p0);
	print_v(p0);
	f_vector p1 = derivative_v(p0);
	row_sthurm.push_back(p1);
	print_v(p1);
	f_vector pn;
	auto k = row_sthurm.size();
	...
	//Видно что можно воспользоватся методом половинного деления.
